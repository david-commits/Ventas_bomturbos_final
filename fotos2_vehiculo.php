...
if ($svvarlor1 != "") {
    if(is_uploaded_file($_FILES['files']['tmp_name']) && $_FILES['files']['type']<300000) {
        $ruta_destino = "fotos_vehiculo/";
        $namefinal="1vehiculo".$_SESSION['id_vehiculo'].".jpg"; //linea nueva devuelve la cadena sin espacios al principio o al final
        $uploadfile=$ruta_destino.$namefinal;
        if(move_uploaded_file($_FILES['files']['tmp_name'], $uploadfile)) {
            $dml="update vehiculos
            set foto1='".$namefinal."',web='".$web."' , fotoprincipal=".$nuevovalor."
            where d_vehiculo=".$_SESSION['id_vehiculo'];
...
if ($svvarlor2 != "") {
    if(is_uploaded_file($_FILES['files1']['tmp_name'])&& $_FILES['files1']['type']<300000) {
        $ruta_destino = "fotos_vehiculo/";
        $namefinal="2vehiculo".$_SESSION['id_vehiculo'].".jpg"; //linea nueva devuelve la cadena sin espacios al principio o al final
        $uploadfile=$ruta_destino.$namefinal;
        if(move_uploaded_file($_FILES['files1']['tmp_name'], $uploadfile)) {
            $dml="update vehiculos
             set foto2='".$namefinal."',web='".$web."', fotoprincipal=".$nuevovalor."
            where d_vehiculo=".$_SESSION['id_vehiculo'];
            $rs=mysqli_query($con,$dml) OR die(mysqli_error($con));
            if(!rs){
                die("No se pudo actualizar..");
...
if ($svvarlor3 != "") {
    if(is_uploaded_file($_FILES['files2']['tmp_name'])&& $_FILES['files2']['type']<300000) {
        $ruta_destino = "fotos_vehiculo/";
        $namefinal="3vehiculo".$_SESSION['id_vehiculo'].".jpg"; //linea nueva devuelve la cadena sin espacios al principio o al final
        $uploadfile=$ruta_destino.$namefinal;
        if(move_uploaded_file($_FILES['files2']['tmp_name'], $uploadfile)) {
            $dml="update vehiculos 
            set foto3='".$namefinal."',web='".$web."', fotoprincipal=".$nuevovalor." 
            where d_vehiculo=".$_SESSION['id_vehiculo'];
            $rs=mysqli_query($con,$dml) OR die(mysqli_error($con));
            if(!rs){
                die("No se pudo actualizar..");
...
if ($svvarlor4 != "") {
   if(is_uploaded_file($_FILES['files3']['tmp_name'])&& $_FILES['files3']['type']<300000) {
        $ruta_destino = "fotos_vehiculo/";
        $namefinal="4vehiculo".$_SESSION['id_vehiculo'].".jpg"; //linea nueva devuelve la cadena sin espacios al principio o al final
        $uploadfile=$ruta_destino.$namefinal;
        if(move_uploaded_file($_FILES['files3']['tmp_name'], $uploadfile)) {
            $dml="update vehiculos set foto4='".$namefinal."',web='".$web."', fotoprincipal=".$nuevovalor." where d_vehiculo=".$_SESSION['id_vehiculo'];
            $rs=mysqli_query($con,$dml) OR die(mysqli_error($con));
            if(!rs){
                die("No se pudo actualizar..");
            }else{
...
if ($nuevovalor != "") {
    $dml="update vehiculos set fotoprincipal=".$nuevovalor." where d_vehiculo=".$_SESSION['id_vehiculo'];
            $rs=mysqli_query($con,$dml) OR die(mysqli_error($con));
            if(!rs){
                die("No se pudo actualizar..");
            }else{
