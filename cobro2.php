...
if($pago-$f[$i]<=0 and $pago>0){        
    $dml = "UPDATE facturas SET cuenta1=cuenta1+$pago WHERE id_factura=$g[$i] ORDER BY  `facturas`.`id_factura` ASC ";
if(!mysqli_query($con,$dml)){
            die("No se pudo actualizar12..");
        }else{
            header("location:cobros.php?o=1");
        }
...
$pago=$pago-$f[$i];
		...
		$sql="INSERT INTO facturas (numero_factura,fecha_factura,fecha_entrega,ot,id_cliente,baja,id_vendedor,condiciones,total_venta,deuda_total,estado_factura,tienda,ven_com,activo,servicio,moneda,nombre,obs,cuenta1,cuenta2,dias,folio) VALUES ('$numero_factura','$date_added','0','0','$id','0','$vendedor','$condiciones','$pago1','0','$estado_factura','$tienda','3','1','$deuda','$mon1','$cuenta','$obs','0','0','0','0')";
			...
			if ($query_new_insert){
				$sql2="select * from facturas ORDER BY  `facturas`.`id_factura` DESC limit 0,1";
                                ...
                                for ($i=1;$i<=$t;$i++ ){
                                  ...
                                  if($pago1-$f[$i]<=0 and $pago1>0){
                                      $dml1=mysqli_query($con,"INSERT INTO pagos (id_pago,id_factura,pago) VALUES ('$id_factura','$g[$i]','$pago1')");
