...
$sumatoriademotos = 0;
$sumatoriademotosprct = 0;
$sqlprcd = "SELECT * FROM constante WHERE estado = 1";

$traemosdatos = mysqli_query($con, $sqlprcd);

while($rowprcnt=mysqli_fetch_array($sqlprcd))
{
    $prcntaje = $rowprcnt["monto"];
    $cambiodolar = $rowprcnt["dolar"];
}

$sqlprincipal=mysqli_query($con, "select * from  tmp where tmp.session_id='".$session_id."' and session_con_id = '".$nuevasesion_identificador."' and venta_compra = '".$identificadordecompra."'");
while ($rowprctaj=mysqli_fetch_array($sqlprincipal))
{
    $id_productop=$rowprctaj["id_producto"];
    $cantidad=$rowprctaj['cantidad_tmp'];
    $precio_ventap=$rowprctaj['precio_tmp'];
    $sumatoriademotos = $sumatoriademotos + ($rowprctaj['precio_tmp'] * $rowprctaj['cantidad_tmp']);
    $sqlprcdcprct = "SELECT * FROM products WHERE id = '".$id_productop."' and estado = 1";

    $nuevoquery = mysqli_query($con, $sqlprcdcprct);
    while($rowpp = mysqli_fetch_array($nuevoquery))
    {
        $sumatoriademotosprct = $sumatoriademotosprct + ($rowpp['costo_producto'] * $cambiodolar);
    }

}

$sumatoriademotosprct = $sumatoriademotosprct * 1.1;

if ($sumatoriademotosprct > $sumatoriademotos) 
{
    return back();
}
else
...
if($_GET['id_cliente']>0){
    ...
    $accion3=mysqli_query($con, "select * from clientes where (doc='$doc1' or (dni='$doc1' and doc='0')) ");
...
$producto=array();
$codigo=array();
$sql=mysqli_query($con, "select * from  tmp where tmp.session_id='".$session_id."' and session_con_id = '".$nuevasesion_identificador."' and venta_compra = '".$identificadordecompra."'");
//COPIAR LOS PRODUCTOS
$variableconidesdetalle_Factura = "0";
while ($row=mysqli_fetch_array($sql))
{
    ...
    //aca se registra una venta detectada porque en ven_com se le esta dando el valor de 1
	$insert_detail=mysqli_query($con, "INSERT INTO detalle_factura (id_cliente, id_vendedor, numero_factura, ot, id_producto, cantidad, precio_venta, tienda, activo, ven_com, fecha, precio_compra, tipo_doc, inv_ini, moneda, folio) VALUES ('$id_cliente','$id_vendedor','$numero_factura','0','$id_producto','$cantidad','$precio_venta_r','$c','1','1','$date_added','$costo','$tipo','$d','$moneda1','$folio')");
    $variableconidesdetalle_Factura = $variableconidesdetalle_Factura.",".$con->insert_id;
...
//aca se esta registrando otra venta detectada porque en ven_com se le esta dando el valor de 1
$insert=mysqli_query($con,"INSERT INTO facturas (numero_factura, fecha_factura, fecha_entrega, ot, id_cliente, baja, id_vendedor, condiciones, total_venta, deuda_total, estado_factura, tienda, ven_com, activo, servicio, moneda, nombre, obs, cuenta1, cuenta2, dias, folio) VALUES ('$numero_factura','$date','$date','$nro_doc','$id_cliente','0','$id_vendedor','$condiciones','$sumador_total','$deuda','$tipo','$c','1','1','1','$moneda1','$des','$motivo','0','0','$dias','$folio')");
...
$id_factura=$row1["id_factura"];
 ...
 $contenidoidDetalleF = explode(",", $variableconidesdetalle_Factura);
  $countarrayDF = count($contenidoidDetalleF); 
for ($i=0; $i <$countarrayDF ; $i++) { 
        if ($contenidoidDetalleF[$i] > 0) {
            $accionUpdateDF=mysqli_query($con, "UPDATE detalle_factura set id_facturas= $id_factura where id_detalle = $contenidoidDetalleF[$i]");
        }
}
...
$cabecera["DEPARTAMENTO_EMPRESA"]="LIMA";
$cabecera["PROVINCIA_EMPRESA"]="LIMA";
$cabecera["DISTRITO_EMPRESA"]="INDEPENDENCIA";
...
<cbc:PriceAmount currencyID="'.$cabecera["COD_MONEDA"].'">'.$cabecera["PRECIO_DET"].'</cbc:PriceAmount>
    ...
    $doc->save("factura-sin-firmar/$ruc1-$tipo_doc-$folio-$numero_factura.xml");
    chmod("factura-sin-firmar/$ruc1-$tipo_doc-$folio-$numero_factura.xml", 0777);
}
//FIN DE CREACION DE DOCUMENTO XML FACTURAS Y BOLETAS
//DIRECCION DE PDF
header("location:ver_factura.php?id_factura=$id_factura&ppventa1=$preventa1&motivo=$motivo&tipo=$tipo&r=$texto_moneda");
